...
static void	rotation_mouse_arrow(t_env *env)
{
	if (env->key.mouse)
	{
...
int			hook_func(t_env *env)
{
	...
	rotation_mouse_arrow(env);
	step_x = cos(env->dir.x) * MOVE_SPEED;
	step_x *= (env->key.shift ? 1.5 : 1);
	step_y = sin(env->dir.x) * MOVE_SPEED;
	step_y *= (env->key.shift ? 1.5 : 1);
